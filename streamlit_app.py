...
# Google Drive 파일 ID
file_id = '1LiAtLXfp7xOvBxIRIUDWyuWRL12B0Wz8'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://ibb.co/2c8sHDw?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=ALl7Fe_GG3U", start_time=0)
